...
static struct passwd mux_pw; /* server-user password entry */
...
ndc_pty(socket_t fd, char * const args[])
{
	...
	if (!s)
		return;
	s->pid = command_pty(fd, &s->wsz, args);
	ndc_fd_watch(s->pty);
}
...
NDX_DEF(int, telnet_connected, socket_t, fd)
{
	struct mux_state s;
	memset(&s, 0, sizeof(s));
	s.pty = -1;
...
NDX_DEF(int, telnet_parse, socket_t, fd, unsigned char *, input, int, nread)
{
	struct mux_state *s = mux_get(fd);
	if (!s)
		return 0;
...
static int
serve_file(socket_t fd, const char *path, const char *mime)
{
	struct stat st;
	FILE *f;
	char *buf;
	...
	snprintf(clen, sizeof(clen), "%zu", n);
	ndc_header(fd, "Content-Type", mime);
	ndc_header(fd, "Content-Length", clen);
	ndc_head(fd, 200);
	ndc_write(fd, buf, n);
	free(buf);
	ndc_close(fd);
...
static int
handle_terminal(socket_t fd, char *body)
{
	(void)body;
	return serve_file(fd, NDC_HTDOCS "/index.html", "text/html; charset=utf-8");
}
...
{
	(void)body;
	return serve_file(fd, NDC_HTDOCS "/ndc.js", "application/javascript");
}
...
{
	(void)body;
	return serve_file(fd, NDC_HTDOCS "/ndc.css", "text/css");
}
...
{
	(void)body;
	return serve_file(fd, NDC_HTDOCS "/ndc-tty.js", "application/javascript");
}
...
ndx_install(void)
{
	...
	ndc_register_handler("GET:/terminal",   handle_terminal);
